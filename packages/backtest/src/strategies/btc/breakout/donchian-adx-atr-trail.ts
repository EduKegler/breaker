...
interface DonchianAdxAtrTrailParams {
...
const DEFAULT_PARAMS: DonchianAdxAtrTrailParams = {
  donchianPeriod: {
...
export function createDonchianAdxAtrTrail(
  paramOverrides?: Partial<Record<keyof DonchianAdxAtrTrailParams, number>>,
): Strategy {
  const params: Record<string, StrategyParam> = {};
  for (const [key, defaultParam] of Object.entries(DEFAULT_PARAMS)) {
    const override = paramOverrides?.[key as keyof DonchianAdxAtrTrailParams];
  ...
  return {
    name: "BTC 15m Breakout — Donchian ADX ATR-Trail",
    params,
