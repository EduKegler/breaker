...
interface DonchianAdxPartialTpParams {
...
const DEFAULT_PARAMS: DonchianAdxPartialTpParams = {
  donchianPeriod: {
    value: 18, min: 14, max: 40, step: 2, optimizable: true,
    description: "Donchian Channel lookback period (N-bar high/low)",
  },
  adxThreshold: {
    value: 20, min: 15, max: 35, step: 5, optimizable: true,
  ...
  tp1RR: {
    value: 1, min: 1.0, max: 3.0, step: 0.5, optimizable: true,
    description: "TP1 take profit R:R ratio (closes 35% of position)",
  },
  tp2RR: {
    value: 2, min: 2.0, max: 5.0, step: 0.5, optimizable: true,
...
export function createDonchianAdxPartialTp(
  paramOverrides?: Partial<Record<keyof DonchianAdxPartialTpParams, number>>,
): Strategy {
  const params: Record<string, StrategyParam> = {};
  for (const [key, defaultParam] of Object.entries(DEFAULT_PARAMS)) {
    const override = paramOverrides?.[key as keyof DonchianAdxPartialTpParams];
  ...
  return {
    name: "BTC 15m Breakout — Donchian ADX Partial-TP",
    params,
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      // 4H ADX regime — only completed bars (anti-repaint)
      ...
      // LONG: close above Donchian upper + ADX < threshold (consolidation) + volume spike
      ...
      // SHORT: close below Donchian lower + ADX < threshold (consolidation) + volume spike
