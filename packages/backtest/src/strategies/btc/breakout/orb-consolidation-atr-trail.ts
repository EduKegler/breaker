...
interface OrbConsolidationAtrTrailParams {
...
const DEFAULT_PARAMS: OrbConsolidationAtrTrailParams = {
  orbMinutes: {
  ...
  consolidationAtrThreshold: {
    value: 2.0, min: 1.0, max: 4.0, step: 0.5, optimizable: true,
...
export function createOrbConsolidationAtrTrail(
  paramOverrides?: Partial<Record<keyof OrbConsolidationAtrTrailParams, number>>,
): Strategy {
  const params: Record<string, StrategyParam> = {};
  for (const [key, defaultParam] of Object.entries(DEFAULT_PARAMS)) {
    const override = paramOverrides?.[key as keyof OrbConsolidationAtrTrailParams];
  ...
  return {
    name: "BTC 15m Breakout — ORB Consolidation ATR Trail",
    params,
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      if (session) {
        ...
        orbArmed = orbBarsLeft === 0; // single-bar formation arms immediately
      ...
      // --- HTF: 4H consolidation (anti-repaint) ---
    ...
    shouldExit(ctx: StrategyContext): { exit: boolean; comment: string } | null {
      const { index, currentCandle, candles, positionDirection, positionEntryBarIndex, higherTimeframes } = ctx;
      ...
      if (positionDirection === "long") {
        ...
        for (let k = positionEntryBarIndex; k <= index; k++) {
          highestHigh = Math.max(highestHigh, candles[k].h);
        }
        ...
        for (let k = positionEntryBarIndex; k <= index; k++) {
          lowestLow = Math.min(lowestLow, candles[k].l);
        }
