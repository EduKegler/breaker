...
interface RangeRetestEmaMacdPartialTpParams {
...
const DEFAULT_PARAMS: RangeRetestEmaMacdPartialTpParams = {
  rangeLookback: {
  ...
  retestWindow: {
    value: 8, min: 4, max: 16, step: 2, optimizable: true,
...
export function createRangeRetestEmaMacdPartialTp(
  paramOverrides?: Partial<Record<keyof RangeRetestEmaMacdPartialTpParams, number>>,
): Strategy {
  const params: Record<string, StrategyParam> = {};
  for (const [key, defaultParam] of Object.entries(DEFAULT_PARAMS)) {
    const override = paramOverrides?.[key as keyof RangeRetestEmaMacdPartialTpParams];
  ...
  // Retest state: tracks pending breakout awaiting pullback confirmation
  let pendingRetest: {
  ...
  return {
    name: "BTC 15m Breakout — Range Retest EMA MACD Partial-TP",
    params,
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      if (pendingRetest) {
        ...
        if (barsSinceBreakout > retestWin) {
          // Window expired — fall through to check new breakout
          pendingRetest = null;
        } else {
          ...
          if (dir === "long") {
            // Retest: candle wick touches or crosses below level, close holds above
            const retestTouch = ctx.track("L:retest_touch", currentCandle.l <= level, currentCandle.l, level);
            ...
            // Invalidate if price dropped too far below level (failed breakout)
            ...
            // Short retest: candle wick touches or crosses above level, close holds below
