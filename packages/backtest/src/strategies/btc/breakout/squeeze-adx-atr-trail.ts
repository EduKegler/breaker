...
interface SqueezeAdxAtrTrailParams {
...
const DEFAULT_PARAMS: SqueezeAdxAtrTrailParams = {
  bbKcPeriod: {
    value: 22, min: 14, max: 30, step: 2, optimizable: true,
    description: "Shared period for Bollinger Bands and Keltner Channel",
  },
  kcMult: {
    value: 2.0, min: 1.5, max: 2.5, step: 0.25, optimizable: true,
  ...
  adxThreshold: {
    value: 35, min: 20, max: 45, step: 5, optimizable: true,
    description: "ADX(14) 4H max threshold — below = consolidation (pre-breakout)",
  },
  volMultiplier: {
...
export function createSqueezeAdxAtrTrail(
  paramOverrides?: Partial<Record<keyof SqueezeAdxAtrTrailParams, number>>,
): Strategy {
  const params: Record<string, StrategyParam> = {};
  for (const [key, defaultParam] of Object.entries(DEFAULT_PARAMS)) {
    const override = paramOverrides?.[key as keyof SqueezeAdxAtrTrailParams];
  ...
  return {
    name: "BTC 15m Breakout — Squeeze ADX ATR-Trail",
    params,
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      // --- Squeeze detection: any squeeze in lookback window ---
      ...
      // --- LONG: squeeze release + close above KC upper + low ADX + volume spike ---
      ...
      if (longSqueeze && longBreakout && longRegime && longVol) {
        return {
          ...
          takeProfits: [],
          comment: "Squeeze release long (ADX)",
        };
      }

      // --- SHORT: squeeze release + close below KC lower + low ADX + volume spike ---
      ...
      if (shortSqueeze && shortBreakout && shortRegime && shortVol) {
        return {
          ...
          stopLoss: close + stopDist,
          takeProfits: [],
          comment: "Squeeze release short (ADX)",
        };
      }
