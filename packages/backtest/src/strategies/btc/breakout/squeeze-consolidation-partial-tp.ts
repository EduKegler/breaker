...
const CONSOLIDATION_LOOKBACK = 10;
const TP1_PCT = 0.35;
const TP2_PCT = 0.35;
const ATR_TRAIL_MULT = 2.5;

interface SqueezeConsolidationPartialTpParams {
  bbKcPeriod: StrategyParam;
  kcMult: StrategyParam;
  consolidationWidthMult: StrategyParam;
...
const DEFAULT_PARAMS: SqueezeConsolidationPartialTpParams = {
  bbKcPeriod: {
  ...
  kcMult: {
    ...
    description: "Keltner Channel multiplier (wider KC = easier squeeze detection)",
  },
  consolidationWidthMult: {
    value: 3.0, min: 2.0, max: 4.0, step: 0.5, optimizable: true,
    description: "Max 4H range/ATR ratio for consolidation regime (lower = stricter)",
  },
  volMultiplier: {
...
export function createSqueezeConsolidationPartialTp(
  paramOverrides?: Partial<Record<keyof SqueezeConsolidationPartialTpParams, number>>,
): Strategy {
  const params: Record<string, StrategyParam> = {};
  for (const [key, defaultParam] of Object.entries(DEFAULT_PARAMS)) {
    const override = paramOverrides?.[key as keyof SqueezeConsolidationPartialTpParams];
  ...
  return {
    name: "BTC 15m Breakout — Squeeze Consolidation Partial-TP",
    params,
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      const kcMid = kc.mid[index];

      if (isNaN(bbUpper) || isNaN(kcUpper) || isNaN(kcMid)) return null;

      // --- Squeeze detection: recent squeeze in lookback window ---
      ...
      const hadRecentSqueeze = recentSqueezeCount >= 2;
      ...
      const consThresholdAbs = params.consolidationWidthMult.value * atr4h;
      ...
      // --- LONG: squeeze release + close above KC upper + 4H consolidation + volume ---
      const longSqueeze = ctx.track("L:squeeze_release", squeezeReleased, recentSqueezeCount, 2);
      ...
      // --- SHORT: squeeze release + close below KC lower + 4H consolidation + volume ---
      const shortSqueeze = ctx.track("S:squeeze_release", squeezeReleased, recentSqueezeCount, 2);
