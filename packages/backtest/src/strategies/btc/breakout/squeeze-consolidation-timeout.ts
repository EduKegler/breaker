...
const SQUEEZE_LOOKBACK = 15;
const SQUEEZE_MIN_BARS = 2;
const FRESH_RELEASE_BARS = 5;
const CONSOLIDATION_LOOKBACK = 8;

interface SqueezeConsolidationTimeoutParams {
...
const DEFAULT_PARAMS: SqueezeConsolidationTimeoutParams = {
  bbKcPeriod: {
    value: 14, min: 14, max: 30, step: 2, optimizable: true,
    description: "Shared period for Bollinger Bands and Keltner Channel",
  },
  kcMult: {
    value: 2, min: 1.0, max: 2.5, step: 0.25, optimizable: true,
    description: "Keltner Channel multiplier (wider = more squeeze events)",
  },
  consWidthMult: {
    value: 4.0, min: 2.0, max: 5.0, step: 0.5, optimizable: true,
...
export function createSqueezeConsolidationTimeout(
  paramOverrides?: Partial<Record<keyof SqueezeConsolidationTimeoutParams, number>>,
): Strategy {
  const params: Record<string, StrategyParam> = {};
  for (const [key, defaultParam] of Object.entries(DEFAULT_PARAMS)) {
    const override = paramOverrides?.[key as keyof SqueezeConsolidationTimeoutParams];
  ...
  return {
    name: "BTC 15m Breakout — Squeeze Consolidation Timeout",
    params,
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      // --- Squeeze detection: count recent squeeze bars in lookback ---
      ...
      // --- LONG: squeeze release + close > KC upper + 4H consolidation + volume ---
      ...
      // --- SHORT: squeeze release + close < KC lower + 4H consolidation + volume ---
