...
interface PctBandsRsi35MaFlatTimeoutParams {
...
const DEFAULT_PARAMS: PctBandsRsi35MaFlatTimeoutParams = {
  maPeriod: {
    value: 20, min: 10, max: 40, step: 2, optimizable: true,
    description: "SMA period for percentage bands midline",
  },
  bandPct: {
    value: 0.75, min: 0.5, max: 3.0, step: 0.25, optimizable: true,
    description: "Percentage distance from SMA to upper/lower band (0.75 = ±0.75%)",
  },
  rsiPeriod: {
  ...
  rsiThreshLong: {
    value: 30, min: 15, max: 35, step: 5, optimizable: true,
    description: "RSI must be below this for long entry (short threshold = 100 - this)",
  },
  maSlopeThresh: {
    value: 0.30, min: 0.05, max: 0.5, step: 0.05, optimizable: true,
    description: "Max absolute % change per bar of 1H SMA(50) for MA-flat regime gate",
  },
  timeoutBarsLong: {
    value: 20, min: 8, max: 48, step: 4, optimizable: true,
    description: "Forced exit after N bars for longs (shorts = N × 0.67)",
  },
};

export function createPctBandsRsi35MaFlatTimeout(
  paramOverrides?: Partial<Record<keyof PctBandsRsi35MaFlatTimeoutParams, number>>,
): Strategy {
  const params: Record<string, StrategyParam> = {};
  for (const [key, defaultParam] of Object.entries(DEFAULT_PARAMS)) {
    const override = paramOverrides?.[key as keyof PctBandsRsi35MaFlatTimeoutParams];
  ...
  /** Anti-repaint: find last two fully-closed 1H candle SMA values for slope calc */
  ...
  return {
    name: "BTC 15m Mean Reversion — Pct Bands RSI35 MA Flat Timeout",
    params,
    requiredTimeframes: ["1h"],
    requiredWarmup: { source: 50, "1h": 80 },

    init(candles: Candle[], higherTimeframes: Record<string, Candle[]>): void {
      const period = Math.round(params.maPeriod.value);
      const rsiPer = Math.round(params.rsiPeriod.value);
      smaCache = sma(candles.map((c) => c.c), period);
      rsiCache = rsi(candles.map((c) => c.c), rsiPer);
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      const period = Math.round(params.maPeriod.value);
      const rsiPer = Math.round(params.rsiPeriod.value);
      if (index < Math.max(period, rsiPer) + 1) return null;
      ...
      // HTF: 1H MA slope regime gate (anti-repaint — last closed 1H candle)
      const htf1hRef = htf1hCandles ?? higherTimeframes["1h"];
      if (!htf1hRef || htf1hRef.length < 55) return null;
      ...
      // --- LONG: close at/below lower band + RSI oversold + MA flat (ranging) ---
      ...
      if (longBand && longRsi && longRegime) {
        return {
          ...
          takeProfits: [],
          comment: "Pct band lower + RSI oversold + MA flat (MR long)",
        };
      }

      // --- SHORT: close at/above upper band + RSI overbought + MA flat (ranging) ---
      ...
      if (shortBand && shortRsi && shortRegime) {
        return {
          ...
          stopLoss: close + virtualStopDist,
          takeProfits: [],
          comment: "Pct band upper + RSI overbought + MA flat (MR short)",
        };
      }
