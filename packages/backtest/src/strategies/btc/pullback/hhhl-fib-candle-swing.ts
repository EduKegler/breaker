...
interface HhhlFibCandleSwingParams {
...
export function createHhhlFibCandleSwing(
  ...
  return {
    name: "BTC 15m Pullback — HHHL Fib Candle Swing",
    params,
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      if (isUptrend) {
        ...
        if (inPullbackZone && pullbackDirection === "long" && activeSwingHigh && activeSwingLow) {
          ...
          if (confirmed) {
            const activeFibDeep = activeSwingHigh.price - fibDeep * (activeSwingHigh.price - activeSwingLow.price);
            ...
            if (withinCap && validStop) {
              ...
              if (rrOk) {
                const retracePct = ((activeSwingHigh.price - close) / (activeSwingHigh.price - activeSwingLow.price) * 100).toFixed(0);
                resetPullback();
                return {
                  direction: "long",
                  entryPrice: null,
                  ...
                  comment: `Pullback long: ${engulfing ? "engulfing" : "hammer"} at Fib ${retracePct}%`,
                };
              }
            }
            resetPullback();
      ...
      if (isDowntrend) {
        ...
        if (inPullbackZone && pullbackDirection === "short" && activeSwingHigh && activeSwingLow) {
          ...
          if (confirmed) {
            const activeFibDeep = activeSwingLow.price + fibDeep * (activeSwingHigh.price - activeSwingLow.price);
            const stopPrice = activeFibDeep + ATR_BUFFER * atr1h;
            ...
            if (withinCap && validStop) {
              ...
              if (rrOk) {
                const retracePct = ((close - activeSwingLow.price) / (activeSwingHigh.price - activeSwingLow.price) * 100).toFixed(0);
                resetPullback();
                return {
                  direction: "short",
                  entryPrice: null,
                  stopLoss: stopPrice,
                  takeProfits: [{ price: tpPrice, pctOfPosition: 1.0 }],
                  comment: `Pullback short: ${engulfing ? "engulfing" : "hammer"} at Fib ${retracePct}%`,
                };
              }
            }
            resetPullback();
