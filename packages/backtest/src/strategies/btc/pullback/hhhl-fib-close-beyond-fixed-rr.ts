...
interface HhhlFibCloseBeyondFixedRrParams {
...
const DEFAULT_PARAMS: HhhlFibCloseBeyondFixedRrParams = {
  swingLookback: {
  ...
  rrTarget: {
    value: 2, min: 1.5, max: 4, step: 0.25, optimizable: true,
...
export function createHhhlFibCloseBeyondFixedRr(
  ...
  return {
    name: "BTC 15m Pullback — HHHL Fib Close Beyond Fixed RR",
    params,
          ...
          if (inPullbackZone && pullbackDirection === "long" && activeSwingHigh && activeSwingLow) {
            ...
            if (confirmed) {
              ...
              if (withinCap && validStop) {
                ...
                const rrTargetVal = params.rrTarget.value;
                const tpPrice = close + rrTargetVal * stopDist;
                const rrOk = ctx.track("L:rr_filter", rrTargetVal >= MIN_RR, rrTargetVal, MIN_RR);
                ...
                if (rrOk) {
                  ...
                  return {
                    ...
                    comment: `Pullback long: close-beyond at Fib ${retracePct}%, RR=${rrTargetVal}`,
                  };
                }
              }
              resetPullback();
          ...
          if (inPullbackZone && pullbackDirection === "short" && activeSwingHigh && activeSwingLow) {
            ...
            if (confirmed) {
              ...
              if (withinCap && validStop) {
                ...
                const rrTargetVal = params.rrTarget.value;
                const tpPrice = close - rrTargetVal * stopDist;
                const rrOk = ctx.track("S:rr_filter", rrTargetVal >= MIN_RR, rrTargetVal, MIN_RR);
                ...
                if (rrOk) {
                  ...
                  return {
                    ...
                    stopLoss: stopPrice,
                    takeProfits: [{ price: tpPrice, pctOfPosition: 1.0 }],
                    comment: `Pullback short: close-beyond at Fib ${retracePct}%, RR=${rrTargetVal}`,
                  };
                }
              }
              resetPullback();
    ...
    computeLevels(ctx: StrategyContext, direction: "long" | "short") {
      ...
      if (direction === "long") {
        ...
        return {
          stopLoss: stopPrice,
          takeProfits: [{ price: close + rrTargetVal * stopDist, pctOfPosition: 1.0 }],
        };
      }
      const fibDeepPrice = lastSL.price + fibDeep * swingRange;
      const stopPrice = fibDeepPrice + ATR_BUFFER * atr1h;
