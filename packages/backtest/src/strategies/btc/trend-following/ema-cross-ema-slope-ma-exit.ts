...
interface EmaCrossEmaSlopeMaExitParams {
...
const DEFAULT_PARAMS: EmaCrossEmaSlopeMaExitParams = {
  emaFast: {
    value: 10,
    min: 9,
    max: 30,
...
export function createEmaCrossEmaSlopeMaExit(
  ...
  return {
    name: "BTC 4H Trend Following — EMA Cross EMA Slope MA Exit",
    params,
    ...
    init(candles: Candle[], higherTimeframes: Record<string, Candle[]>): void {
      ...
      entryFastCache = ema(closes4h, params.emaFast.value);
      entrySlowCache = ema(closes4h, params.emaSlow.value);

      // Exit crossover EMAs on 4H (separate from entry)
      exitFastCache = ema(closes4h, MA_EXIT_FAST_PERIOD);
      exitSlowCache = ema(closes4h, params.maExitSlow.value);
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      const fastVals = entryFastCache;
      const slowVals = entrySlowCache;
    ...
    shouldExit(ctx: StrategyContext): { exit: boolean; comment: string } | null {
      ...
      const fastEma = exitFastCache;
      const slowEma = exitSlowCache;
