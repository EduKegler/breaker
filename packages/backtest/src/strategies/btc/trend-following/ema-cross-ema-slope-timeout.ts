...
interface EmaCrossEmaSlopeTimeoutParams {
...
const DEFAULT_PARAMS: EmaCrossEmaSlopeTimeoutParams = {
  emaFast: {
    value: 10,
    min: 9,
    max: 30,
...
export function createEmaCrossEmaSlopeTimeout(
  ...
  return {
    name: "BTC 4H Trend Following — EMA Cross EMA Slope Timeout",
    params,
    ...
    shouldExit(ctx: StrategyContext): { exit: boolean; comment: string } | null {
      ...
      // No trailing exit — timeout-only variant.
      // Hard SL handled by engine via computeLevels.
      return null;
    },

    getExitLevel(_ctx: StrategyContext): number | null {
      // No trailing stop level — exits only via timeout or hard SL
      return null;
    },
