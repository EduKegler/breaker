 ...
 * Not a tunable param because timeout-only exit has no trailing exit to constrain —
 * both SL and timeout are exempt from min-hold.
 */
const MIN_HOLD_BARS = 3;

interface LongMacdHtfAdxHighTimeoutParams {
...
const DEFAULT_PARAMS: LongMacdHtfAdxHighTimeoutParams = {
  macdFast: {
    value: 12,
    min: 8,
    max: 20,
...
export function createLongMacdHtfAdxHighTimeout(
  ...
  return {
    name: "BTC 4H Trend Following — Long MACD HTF ADX High Timeout",
    params,
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      // LONG ONLY: 4H MACD cross above signal + Daily MACD above signal (bullish) + ADX
    ...
    shouldExit(ctx: StrategyContext): { exit: boolean; comment: string } | null {
      ...
      // No trailing exit — timeout-only variant. SL handled by engine via computeLevels.
      return null;
    },

    getExitLevel(_ctx: StrategyContext): number | null {
      // No trailing exit level — timeout-only strategy
      return null;
    },
