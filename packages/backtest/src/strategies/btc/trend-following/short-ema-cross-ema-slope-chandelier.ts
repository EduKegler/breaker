...
interface ShortEmaCrossEmaSlopeChandelierParams {
...
const DEFAULT_PARAMS: ShortEmaCrossEmaSlopeChandelierParams = {
  emaFast: {
    value: 10,
    min: 9,
    max: 30,
...
export function createShortEmaCrossEmaSlopeChandelier(
  ...
  return {
    name: "BTC 4H Trend Following — Short EMA Cross EMA Slope Chandelier",
    params,
    ...
    init(candles: Candle[], higherTimeframes: Record<string, Candle[]>): void {
      ...
      // Chandelier Exit on 4H (trailing exit for shorts)
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      if (chandelierCache) {
        ctx.indicator("chandelierShort", chandelierCache.shortExit[index]);
      }
    ...
    shouldExit(ctx: StrategyContext): { exit: boolean; comment: string } | null {
      ...
      // ── Chandelier trailing exit on 4H (short positions) ──
      if (!chandelierCache) return null;

      const chandelierLevel = chandelierCache.shortExit[index];
    ...
    getExitLevel(ctx: StrategyContext): number | null {
      ...
      const level = chandelierCache.shortExit[index];
