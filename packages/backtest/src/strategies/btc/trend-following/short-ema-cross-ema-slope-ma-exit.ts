...
interface ShortEmaCrossEmaSlopeMaExitParams {
...
const DEFAULT_PARAMS: ShortEmaCrossEmaSlopeMaExitParams = {
  emaFast: {
    value: 10,
    min: 9,
    max: 30,
  ...
  maExitSlow: {
    ...
    optimizable: true,
    description:
      "Slow EMA period for MA crossover exit on 4H (fast=emaFast shared)",
  },
  timeoutBars: {
    value: 72,
    min: 60,
...
export function createShortEmaCrossEmaSlopeMaExit(
  ...
  return {
    name: "BTC 4H Trend Following — Short EMA Cross EMA Slope MA Exit",
    params,
    ...
    init(candles: Candle[], higherTimeframes: Record<string, Candle[]>): void {
      ...
      // Entry crossover EMAs on 4H
      fastEmaCache = ema(closes4h, params.emaFast.value);
      slowEmaCache = ema(closes4h, params.emaSlow.value);

      // MA exit slow EMA on 4H (exit crossover uses fastEmaCache as fast component)
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      if (maExitSlowCache) {
        ctx.indicator("maExitSlow4h", maExitSlowCache[index]);
      }

      // ── SHORT ONLY: fast EMA crosses below slow + Daily EMA slope falling + price below EMA ──
    ...
    shouldExit(ctx: StrategyContext): { exit: boolean; comment: string } | null {
      ...
      // ── MA crossover exit on 4H (short positions) ──
      // Exit when fast EMA crosses above maExitSlow EMA (bearish momentum ended)
      if (!fastEmaCache || !maExitSlowCache) return null;

      const currFastExit = fastEmaCache[index];
      const prevFastExit = fastEmaCache[index - 1];
      const currSlowExit = maExitSlowCache[index];
      const prevSlowExit = maExitSlowCache[index - 1];
      ...
      ctx.indicator("maExitFast", currFastExit);
      ctx.indicator("maExitSlow", currSlowExit);

      // For short: exit on golden cross (fast crosses above slow = bearish trend over)
      if (prevFastExit <= prevSlowExit && currFastExit > currSlowExit) {
    ...
    getExitLevel(_ctx: StrategyContext): number | null {
      // MA crossover exit doesn't have a single trigger price level.
      // Return null — the exit is condition-based, not level-based.
      return null;
    },
