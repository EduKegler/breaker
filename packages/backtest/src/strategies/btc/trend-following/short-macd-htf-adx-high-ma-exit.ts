...
interface ShortMacdHtfAdxHighMaExitParams {
...
const DEFAULT_PARAMS: ShortMacdHtfAdxHighMaExitParams = {
  macdFast: {
    value: 12,
    min: 8,
    max: 20,
  ...
  maExitFast: {
    ...
    optimizable: true,
    description:
      "Fast EMA period for MA crossover exit on 4H (slow=21 fixed). Golden cross exits short.",
  },
  timeoutBars: {
    value: 90,
    min: 60,
...
export function createShortMacdHtfAdxHighMaExit(
  ...
  return {
    name: "BTC 4H Trend Following — Short MACD HTF ADX High MA Exit",
    params,
    ...
    onCandle(ctx: StrategyContext): Signal | null {
      ...
      // SHORT ONLY: 4H MACD cross below signal + Daily MACD below signal (bearish) + ADX
      ...
      if (shortCross && shortAlign && shortAdx) {
        return {
          ...
          takeProfits: [],
          comment: "MACD HTF cross short (Daily bearish + ADX confirmed)",
        };
      }
    ...
    shouldExit(ctx: StrategyContext): { exit: boolean; comment: string } | null {
      ...
      // MA crossover exit on 4H: golden cross = fast EMA crosses above slow EMA → exits short
      const closes4h = candles.map((c) => c.c);
      ...
      if (positionDirection === "short" && prevFast <= prevSlow && currFast > currSlow) {
    ...
    getExitLevel(ctx: StrategyContext): number | null {
      ...
      // Return the slow EMA as the trailing resistance level for shorts
      const closes4h = candles.map((c) => c.c);
      const slowVals = slowEmaCache ?? ema(closes4h, MA_EXIT_SLOW);
      const level = slowVals[index];

      if (positionDirection === "short" && !isNaN(level)) {
        return level;
      }
