...
- Build command: \`pnpm --filter @trading/backtest typecheck\`
...
4. **Run** \`pnpm --filter @trading/backtest typecheck\` to verify the fix compiles
...
- The strategy must still conform to the \`Strategy\` interface from @trading/backtest
- If a type import is missing, add it from @trading/backtest
`;
}
