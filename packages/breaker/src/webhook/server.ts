...
// Logging
// ---------------------
// In Docker: /app/dist/webhook → ../../logs = /app/logs (matches volume mount)
// Local: src/webhook → ../../infra/logs (existing behavior)
...
export function isDuplicate(alertId: string): boolean {
  const now = Date.now();
  // Evict expired entries periodically (on every check)
...
// ---------------------
async function sendWhatsApp(text: string): Promise<unknown> {
  const url = `${EVOLUTION_API_URL}/message/sendText/${EVOLUTION_INSTANCE}`;
  const body = JSON.stringify({
    number: WHATSAPP_RECIPIENT,
    text,
  });
...
async function sendWithRetry(text: string): Promise<unknown> {
  try {
    ...
    log("warn", "WhatsApp send failed, retrying in 5s", {
...
// ---------------------
// Constant-time secret comparison (H3)
// ---------------------
...
// ---------------------
// Rate limiter (H1) — in-memory, per-IP, no deps
// ---------------------
...
// Shared webhook handler — token already validated by route
async function handleWebhook(req: express.Request, res: express.Response): Promise<void> {
  ...
  // TTL check (BEFORE idempotency — expired alerts must not pollute the cache)
  ...
  // Idempotency check — Redis first, fallback to in-memory
  ...
  try {
    ...
    // Mark as sent ONLY after successful delivery (both Redis + memory)
    ...
    // Return 502 so TradingView retries the alert
    res.status(502).json({
      status: "send_failed",
      alert_id: typedAlert.alert_id,
    });
...
// Token-based route: /webhook/:token (secret in URL, no secret needed in body)
...
// Legacy route: /webhook (secret in body — backward compatible)
...
app.post("/debug", rateLimit(5), (req, res) => {
  // Require secret to access debug endpoint
...
if (isMain) {
  const bootstrap = async (): Promise<void> => {
    ...
    if (!WHATSAPP_RECIPIENT) {
      console.error(
        "WARNING: WHATSAPP_RECIPIENT not set — alerts will NOT be delivered",
      );
    }
    if (!EVOLUTION_API_KEY) {
      console.error(
        "WARNING: EVOLUTION_API_KEY not set — WhatsApp API calls will fail",
      );
    }
    if (!WEBHOOK_SECRET) {
      console.error(
    ...
    app.listen(PORT, () => {
      log("info", `Webhook server started on port ${PORT}`, {
        ttl: TTL_SECONDS,
        recipient: WHATSAPP_RECIPIENT ? "configured" : "MISSING",
