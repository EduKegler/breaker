...
describe("Exchange server", () => {
  ...
  describe("POST /quick-signal", () => {
    ...
    it("uses computeLevels from strategy via generateManualSignal", async () => {
      const candles = makeCandles(30);
      btcStreamer.getCandles.mockReturnValue(candles);
      ...
      expect(res.status).toBe(200);
      expect(res.body.status).toBe("executed");
      // SL comes from the strategy's computeLevels (93500), not ATR
      expect(res.body.stopLoss).toBe(93500);
    });

    it("falls back to ATR when generateManualSignal returns null", async () => {
      const candles = makeCandles(30);
      btcStreamer.getCandles.mockReturnValue(candles);
